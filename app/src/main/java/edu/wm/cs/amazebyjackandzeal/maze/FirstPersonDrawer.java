...
import edu.wm.cs.amazebyjackandzeal.maze.Constants.StateGUI;
import edu.wm.cs.amazebyjackandzeal.maze.MazePanel.MazePanelColors;
import edu.wm.cs.amazebyjackandzeal.generation.BSPBranch;
import edu.wm.cs.amazebyjackandzeal.generation.BSPLeaf;
import edu.wm.cs.amazebyjackandzeal.generation.BSPNode;
import edu.wm.cs.amazebyjackandzeal.generation.Cells;
import edu.wm.cs.amazebyjackandzeal.generation.Seg;

import java.util.ArrayList;

// memo: update to get into version 116 again
/**
...
public class FirstPersonDrawer extends DefaultViewer {
    ...
    private MazePanel mazePanel ; // the graphics object for the buffer image this class draws on
     ...
     */
    @Override
    public void redraw(MazePanel mazePanel, int px, int py,
        ...
        Log.v("FirstPersonDrawer", "redraw called!");
    ...
    private void drawBackground(MazePanel mazePanel) {
        Log.v("FirstPersonDrawer", "DRAWING BG");
        mazePanel.setColor(MazePanelColors.BLACK);
        mazePanel.fillRect(0, 0, 400, 400);
        mazePanel.setColor(MazePanelColors.DARKGRAY);
        mazePanel.fillRect(0, 200, 400, 200);
    }
    /**
    ...
    private boolean boundingBoxIsVisible(BSPNode b) {
        ...
        // constraint: x1 <= x2
        return (rset.intersect(new int[] {x1, x2}));
    }
    ...
    private void drawSegment(Seg seg) {
        ...
        int x1i = x1;
        int xd = x2-x1;
        mazePanel.setColor(seg.getRed(), seg.getGreen(), seg.getBlue());
        boolean drawn = false;
    ...
    private void udpateSeenCellsForSegment(Seg seg) {
	    /* original code
	    // TODO: move method to Cells.java class: seencells.addSegment(Seg seg)
		...
		if (sdx < 0) // this direction and negative
			sx--;
		int sy = seg.getStartPositionY() / map_unit;
		if (sdy < 0) // this direction and negative
			sy--;
		...
		*/
        // new code after relocation, inline if stable
        seencells.addWallsForSegment(seg, map_unit);
    }
    ...
    static private boolean clipt(int denom, int num, FloatPair fp) {
        // original code
        // TODO: make this a method of class FloatPair
        // Why: fp gets manipulated, return value is a boolean
	    /*
		if (denom > 0) {
    ...
    protected boolean clip3d(RangePair rp) {
        // original code
        // TODO: make this method a local method of the RangePair class
        // Why: only rp gets modified and result is a boolean
	    /*
